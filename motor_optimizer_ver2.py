# motor_optimizer_ver2.py
"""
Version 2 of the AI optimizer for the V‑Shape IPM motor.

Key enhancements compared to `motor_optimizer.py`:

1. **Explicit ordering guarantee** – the order of variables is read once from
   `Ai_Optimization_Bounds.xlsx` and stored in a global `PARAM_ORDER` list.
   Both the Excel writer and the MATLAB reader use this list, ensuring that a
   value is never written to the wrong column.
2. **Checkpoint / resume** – after each generation the current population,
   best‑so‑far design and GA parameters are saved to `optimizer_state.pkl`.
   The script can be re‑started with `--resume` to continue from the last
   checkpoint without losing progress.
3. **Improved constraint handling** – all geometric constraints are expressed
   as callable functions in `CONSTRAINTS`.  The GA now uses a *repair* step that
   attempts to fix any violated constraint by snapping the offending variables
   back into the feasible region before discarding the individual.
4. **Modular fitness evaluation** – two evaluation back‑ends are offered:
   * `matlab` – launches MATLAB (batch mode) exactly as before.
   * `offline` – a lightweight analytical surrogate that mimics the shape of the
     true objective (higher efficiency, lower torque ripple) while still
     respecting the variable bounds.  This mode can be used for rapid prototyping
     when the Ansys/Matlab tool‑chain is unavailable.
5. **Detailed logging** – a per‑generation log file `optimizer.log` records the
   best score, efficiency, torque ripple, cost and power density for every
   generation.  This assists later post‑processing and reproducibility.
6. **Command‑line interface** – additional flags for custom population size,
   number of generations, mutation/crossover rates, seed control and the
...
def offline_surrogate(ind: dict,
                       ...
                       w_ripple: float = 1.0,
                       w_pwr: float    = 0.5,
                       w_cost: float   = 0.05) -> dict:
    """Improved analytical surrogate evaluation.
...
def evaluate_population(population: list, root_dir: Path, input_dir: Path,
                         ...
                         score_weights: dict | None = None) -> list:
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="V-Shape IPM motor GA optimizer (v2)")
    parser.add_argument("--pop-size",   type=int,   default=8,       help="Population size")
    parser.add_argument("--generations",type=int,   default=10,      help="Max number of generations")
    ...
    # -------------------------------------------------------------------
    # Setup logging – a per‑run log file plus console output in output/
    # -------------------------------------------------------------------
    ...
    for gen in range(start_gen, args.generations + 1):
        ...
        metrics = evaluate_population(population, root_dir, input_dir,
                                      output_dir, args.mode, score_weights)
    ...
    out_path = output_dir / "best_optimized_design_v2.csv"
