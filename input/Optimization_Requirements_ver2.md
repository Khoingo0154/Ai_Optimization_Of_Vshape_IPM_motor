...
| D1 | Minimum diameter of Permanent magnet ducts (in same pole) | Calculated | Ds_in - 2*air_gap - 2*bridge | - | - | - |
...
**Decision note:** Determined based on min{Ds_in} so we don't lose valuable samples on optimization and when it's too big it just won't run that model.
