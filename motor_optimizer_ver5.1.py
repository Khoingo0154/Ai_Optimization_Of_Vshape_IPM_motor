...
def is_feasible(params: dict) -> bool:
    """Return True only if all 4 geometric constraints are satisfied."""
...
def evaluate_population(population: List[Dict], 
    ...
    try:
        ...
        logging.error(str(e))
        # Return penalty metrics for all individuals
        return [{"score": -1e6, "efficiency": 0.0, "torque_ripple": 1e6,
                 "cost": 1e9, "power_density": 0.0}] * len(population)
  ...
  python motor_optimizer_ver5.1.py --test
    ...
    for gen in range(start_gen, args.generations + 1):
        ...
        if args.algorithm == "nsga2":
            ...
            # Combine parent + offspring, then environmental selection
